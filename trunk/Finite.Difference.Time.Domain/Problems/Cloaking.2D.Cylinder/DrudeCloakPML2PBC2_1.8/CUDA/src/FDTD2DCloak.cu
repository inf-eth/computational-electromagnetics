...
#include <sstream>
#include <iomanip>
#include <FDTD2DCloak_Kernels.cu>
...
int CFDTD2DCloak::AllocateMemoryGPU()
{
	...
	checkCudaErrors(cudaMalloc((void **)&d_EzMask_, sizeof(PRECISION)*IEz*JEz));
	...
	checkCudaErrors(cudaMalloc((void **)&d_BxAve_, sizeof(PRECISION)*IHy*JHy*3));
	...
	checkCudaErrors(cudaMalloc((void **)&d_ByAve_, sizeof(PRECISION)*IHx*JHx*3));
	// Incident and transmitted fields.
	...
	// Auxiliary field scalars.
	checkCudaErrors(cudaMalloc((void **)&d_ax_, sizeof(PRECISION)*IHx*JHx*9));
	checkCudaErrors(cudaMalloc((void **)&d_ay_, sizeof(PRECISION)*IHy*JHy*9));
	checkCudaErrors(cudaMalloc((void **)&d_az_, sizeof(PRECISION)*IEz*JEz*5));
	// PML arrays.
...
int CFDTD2DCloak::CopyDataCPUtoGPU()
{
	...
	checkCudaErrors(cudaMemcpy(d_EzMask_, EzMask_, sizeof(PRECISION)*IEz*JEz, cudaMemcpyHostToDevice));
	...
	checkCudaErrors(cudaMemcpy(d_BxAve_, BxAve_, sizeof(PRECISION)*IHy*JHy*3, cudaMemcpyHostToDevice));
	...
	checkCudaErrors(cudaMemcpy(d_ByAve_, ByAve_, sizeof(PRECISION)*IHx*JHx*3, cudaMemcpyHostToDevice));
	// Incident and transmitted fields.
	...
	// Auxiliary field scalars.
	checkCudaErrors(cudaMemcpy(d_ax_, ax_, sizeof(PRECISION)*IHx*JHx*9, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_ay_, ay_, sizeof(PRECISION)*IHy*JHy*9, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_az_, az_, sizeof(PRECISION)*IEz*JEz*5, cudaMemcpyHostToDevice));
	// PML arrays.
...
int CFDTD2DCloak::InitialiseForSimulationGPU()
{
	// Data arrays.
	checkCudaErrors(cudaMemcpy(d_Ez_, Ez_, sizeof(PRECISION)*IEz*JEz*3, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_Dz_, Dz_, sizeof(PRECISION)*IEz*JEz*3, cudaMemcpyHostToDevice));
...
int CFDTD2DCloak::DryRunGPU()
{
	...
	for (unsigned int n=0;n<MaxTime; n++)
	{
		...
		// Kernel call.
		FDTD2DCloakKernel_DryRun_M <ThreadsX, ThreadsY> <<<Blocks, Threads>>>(
											I,
											J,
											PMLw,
											PulseWidth,
		...
		FDTD2DCloakKernel_DryRun_E <ThreadsX, ThreadsY> <<<Blocks, Threads>>>(
											I,
											J,
											PMLw,
											PulseWidth,
...
int CFDTD2DCloak::RunSimulationGPU(bool SaveFields)
{
	// Total local threads in a block. Can be thought of as Block dimensions.
	const unsigned int ThreadsX = 16;
	const unsigned int ThreadsY = 16;
	...
	for (unsigned int n=0;n<MaxTime; n++)
	{
		...
		// Kernel call.
		FDTD2DCloakKernel_Simulation_B <ThreadsX, ThreadsY> <<<Blocks, Threads>>>(
											I,
											J,
											PMLw,
											PulseWidth,
		...
		getLastCudaError("Kernel execution failed");
		checkCudaErrors(cudaThreadSynchronize());
		FDTD2DCloakKernel_Simulation_H <ThreadsX, ThreadsY> <<<Blocks, Threads>>>(
											I,
											J,
											PMLw,
											PulseWidth,
											...
											dr,
											IEz, JEz, IHx, JHx, IHy, JHy,
											d_Ez_, d_Dz_, d_Hx_, d_Bx_, d_BxAve_, d_Hy_, d_By_, d_ByAve_,
											d_ax_, d_ay_, d_az_,
											d_PsiEzX_, d_PsiEzY_, d_PsiHyX_, d_PsiHxY_,
											kappex, kappey, kappmx, kappmy,
											bex, bey, bmx, bmy,
		...
		getLastCudaError("Kernel execution failed");
		checkCudaErrors(cudaThreadSynchronize());
		FDTD2DCloakKernel_Simulation_E <ThreadsX, ThreadsY> <<<Blocks, Threads>>>(
											I,
											J,
											PMLw,
											PulseWidth,
											...
											dr,
											IEz, JEz, IHx, JHx, IHy, JHy,
											d_Ez_, d_Dz_, d_EzMask_, d_Hx_, d_Bx_, d_Hy_, d_By_,
											d_ax_, d_ay_, d_az_,
											d_PsiEzX_, d_PsiEzY_, d_PsiHyX_, d_PsiHxY_,
											kappex, kappey, kappmx, kappmy,
											bex, bey, bmx, bmy,
...
int CFDTD2DCloak::CompleteRunGPU(bool SaveFields)
{
	cout << "Memory required for simulation = " << SimSize() << " bytes (" << (double)SimSize()/1024UL << "kB/" << (double)SimSize()/1024UL/1024UL << "MB)." << endl;
	cout << "HDD space required for data storage = " << HDDSpace() << " bytes (" << (double)HDDSpace()/1024UL << "kB/" << (double)HDDSpace()/1024UL/1024UL << "MB)." << endl;
	if (HDDSpace() > 2147483648UL)
...
int CFDTD2DCloak::CleanupGPU()
{
	...
	checkCudaErrors(cudaFree(d_ax_));
	checkCudaErrors(cudaFree(d_ay_));
