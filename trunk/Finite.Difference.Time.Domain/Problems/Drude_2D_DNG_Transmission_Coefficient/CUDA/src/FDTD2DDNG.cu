...
int CFDTD2DDNG::AllocateMemoryGPU()
{
	...
	// Data arrays.
	checkCudaErrors(cudaMalloc((void **)&d_Ez_, sizeof(PRECISION)*IEz*JEz*3));
	checkCudaErrors(cudaMalloc((void **)&d_Dz_, sizeof(PRECISION)*IEz*JEz*3));
	checkCudaErrors(cudaMalloc((void **)&d_Hx_, sizeof(PRECISION)*IHx*JHx*3));
	checkCudaErrors(cudaMalloc((void **)&d_Bx_, sizeof(PRECISION)*IHx*JHx*3));
	checkCudaErrors(cudaMalloc((void **)&d_Hy_, sizeof(PRECISION)*IHy*JHy*3));
	checkCudaErrors(cudaMalloc((void **)&d_By_, sizeof(PRECISION)*IHy*JHy*3));
	// Incident and transmitted fields.
	checkCudaErrors(cudaMalloc((void **)&d_Ezi, sizeof(PRECISION)*MaxTime));
	checkCudaErrors(cudaMalloc((void **)&d_Ezt, sizeof(PRECISION)*MaxTime));
	checkCudaErrors(cudaMalloc((void **)&d_Eztt, sizeof(PRECISION)*MaxTime));
	// Refractive Index.
	checkCudaErrors(cudaMalloc((void **)&d_Ezy1, sizeof(PRECISION)*MaxTime));
	checkCudaErrors(cudaMalloc((void **)&d_Ezy2, sizeof(PRECISION)*MaxTime));
	// Drude material parameters.
	checkCudaErrors(cudaMalloc((void **)&d_einf_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_uinf_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_wpesq_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_wpmsq_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_ge_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_gm_, sizeof(PRECISION)*IHx*JHx));
	// Auxiliary field scalars.
	checkCudaErrors(cudaMalloc((void **)&d_ae0_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_ae_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_be_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_ce_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_de_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_ee_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_am0_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_am_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_bm_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_cm_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_dm_, sizeof(PRECISION)*IHx*JHx));
	checkCudaErrors(cudaMalloc((void **)&d_em_, sizeof(PRECISION)*IHx*JHx));
	// PML arrays.
	checkCudaErrors(cudaMalloc((void **)&d_PsiEzX_, sizeof(PRECISION)*IEz*JEz));
	checkCudaErrors(cudaMalloc((void **)&d_PsiEzY_, sizeof(PRECISION)*IEz*JEz));
	checkCudaErrors(cudaMalloc((void **)&d_PsiHyX_, sizeof(PRECISION)*IHy*JHy));
	checkCudaErrors(cudaMalloc((void **)&d_PsiHxY_, sizeof(PRECISION)*IHx*JHx));

	return 0;
}
int CFDTD2DDNG::CopyDataCPUtoGPU()
{
	// Device data initialisation.

	// Data arrays.
	checkCudaErrors(cudaMemcpy(d_Ez_, Ez_, sizeof(PRECISION)*IEz*JEz*3, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_Dz_, Dz_, sizeof(PRECISION)*IEz*JEz*3, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_Hx_, Hx_, sizeof(PRECISION)*IHx*JHx*3, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_Bx_, Bx_, sizeof(PRECISION)*IHx*JHx*3, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_Hy_, Hy_, sizeof(PRECISION)*IHy*JHy*3, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_By_, By_, sizeof(PRECISION)*IHy*JHy*3, cudaMemcpyHostToDevice));
	// Incident and transmitted fields.
	checkCudaErrors(cudaMemcpy(d_Ezi, Ezi, sizeof(PRECISION)*MaxTime, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_Ezt, Ezt, sizeof(PRECISION)*MaxTime, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_Eztt, Eztt, sizeof(PRECISION)*MaxTime, cudaMemcpyHostToDevice));
	// Refractive Index.
	checkCudaErrors(cudaMemcpy(d_Ezy1, Ezy1, sizeof(PRECISION)*MaxTime, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_Ezy2, Ezy2, sizeof(PRECISION)*MaxTime, cudaMemcpyHostToDevice));
	// Drude material parameters.
	checkCudaErrors(cudaMemcpy(d_einf_, einf_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_uinf_, uinf_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_wpesq_, wpesq_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_wpmsq_, wpmsq_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_ge_, ge_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_gm_, gm_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	// Auxiliary field scalars.
	checkCudaErrors(cudaMemcpy(d_ae0_, ae0_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_ae_, ae_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_be_, be_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_ce_, ce_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_de_, de_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_ee_, ee_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_am0_, am0_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_am_, am_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_bm_, bm_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_cm_, cm_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_dm_, dm_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_em_, em_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
	// PML arrays.
	checkCudaErrors(cudaMemcpy(d_PsiEzX_, PsiEzX_, sizeof(PRECISION)*IEz*JEz, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_PsiEzY_, PsiEzY_, sizeof(PRECISION)*IEz*JEz, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_PsiHyX_, PsiHyX_, sizeof(PRECISION)*IHy*JHy, cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_PsiHxY_, PsiHxY_, sizeof(PRECISION)*IHx*JHx, cudaMemcpyHostToDevice));
...
int CFDTD2DDNG::DryRunGPU()
{
	...
	for (unsigned int n=0;n<MaxTime; n++)
	{
		...
		// Kernel call.
		FDTD2DDNGKernel_DryRun_M <ThreadsX, ThreadsY> <<<Blocks, Threads>>>(
											I,
											J,
											PMLw,
											PulseWidth,
											...
											fp,
											dr,
											IEz, JEz, IHx, JHx, IHy, JHy,
											d_Ez_, d_Dz_, d_Hx_, d_Bx_, d_Hy_, d_By_,
											d_PsiEzX_, d_PsiEzY_, d_PsiHyX_, d_PsiHxY_,
											d_einf_, d_uinf_,
											kappex, kappey, kappmx, kappmy,
											bex, bey, bmx, bmy,
											Cex, Cey, Cmx, Cmy,
											d_Ezi,
											x1,
		...
		FDTD2DDNGKernel_DryRun_E <ThreadsX, ThreadsY> <<<Blocks, Threads>>>(
											I,
											J,
											PMLw,
											PulseWidth,
...
int CFDTD2DDNG::RunSimulationGPU(bool SaveFields)
{
	...
	for (unsigned int n=0;n<MaxTime; n++)
	{
		...
		// Kernel call.
		FDTD2DDNGKernel_Simulation_M <ThreadsX, ThreadsY> <<<Blocks, Threads>>>(
											I,
											J,
											PMLw,
											PulseWidth,
											...
											IEz, JEz, IHx, JHx, IHy, JHy,
											d_Ez_, d_Dz_, d_Hx_, d_Bx_, d_Hy_, d_By_,
											d_einf_, d_uinf_, d_wpesq_, d_wpmsq_, d_ge_, d_gm_,
											d_ae0_, d_ae_, d_be_, d_ce_, d_de_, d_ee_,
											d_am0_, d_am_, d_bm_, d_cm_, d_dm_, d_em_,
											d_PsiEzX_, d_PsiEzY_, d_PsiHyX_, d_PsiHxY_,
											kappex, kappey, kappmx, kappmy,
											bex, bey, bmx, bmy,
											Cex, Cey, Cmx, Cmy,
											d_Ezt, d_Eztt, d_Ezy1, d_Ezy2,
											x1, Y1, Y2,
											n,
											np,
											n0,
		...
		getLastCudaError("Kernel execution failed");
		checkCudaErrors(cudaThreadSynchronize());
		FDTD2DDNGKernel_Simulation_E <ThreadsX, ThreadsY> <<<Blocks, Threads>>>(
											I,
											J,
											PMLw,
											PulseWidth,
		...
		if (n%SnapshotInterval == 0 && SaveFields == true)
		{
			...
			filename = basename + framestream.str() + ".fdt";
			snapshot.open(filename.c_str(), std::ios::out|std::ios::binary);
			checkCudaErrors(cudaMemcpy(Ez_, d_Ez_, sizeof(PRECISION)*IEz*JEz*3, cudaMemcpyDeviceToHost));
			snapshot.write((char*)&(Ez(0,0,nf)), sizeof(PRECISION)*IEz*JEz);
			snapshot.close();
		}
	...
	if (SaveFields == true)
	{
		...
		// Write saved fields to files.
		snapshot.open("FieldData/Ezi.fdt", std::ios::out|std::ios::binary);
		checkCudaErrors(cudaMemcpy(Ezi, d_Ezi, sizeof(PRECISION)*MaxTime, cudaMemcpyDeviceToHost));
		snapshot.write((char*)Ezi, sizeof(PRECISION)*MaxTime);
		snapshot.close();
		snapshot.open("FieldData/Ezt.fdt", std::ios::out|std::ios::binary);
		checkCudaErrors(cudaMemcpy(Ezt, d_Ezt, sizeof(PRECISION)*MaxTime, cudaMemcpyDeviceToHost));
		snapshot.write((char*)Ezt, sizeof(PRECISION)*MaxTime);
		snapshot.close();
		snapshot.open("FieldData/Eztt.fdt", std::ios::out|std::ios::binary);
		checkCudaErrors(cudaMemcpy(Eztt, d_Eztt, sizeof(PRECISION)*MaxTime, cudaMemcpyDeviceToHost));
		snapshot.write((char*)Eztt, sizeof(PRECISION)*MaxTime);
		snapshot.close();
		snapshot.open("FieldData/Ezy1.fdt", std::ios::out|std::ios::binary);
		checkCudaErrors(cudaMemcpy(Ezy1, d_Ezy1, sizeof(PRECISION)*MaxTime, cudaMemcpyDeviceToHost));
		snapshot.write((char*)Ezy1, sizeof(PRECISION)*MaxTime);
		snapshot.close();
		snapshot.open("FieldData/Ezy2.fdt", std::ios::out|std::ios::binary);
		checkCudaErrors(cudaMemcpy(Ezy2, d_Ezy2, sizeof(PRECISION)*MaxTime, cudaMemcpyDeviceToHost));
		snapshot.write((char*)Ezy2, sizeof(PRECISION)*MaxTime);
		snapshot.close();
	}
...
int CFDTD2DDNG::CompleteRunGPU(bool SaveFields)
{
	...
	SafeCall(InitialiseForSimulationCPU(), "Error: Initalising for simulation (CPU).");
	SafeCall(InitialiseForSimulationGPU(), "Error: Initialising for simulation (GPU).");
...
{
	// Device cleanup.
	checkCudaErrors(cudaFree(d_Ez_));
	checkCudaErrors(cudaFree(d_Dz_));
	checkCudaErrors(cudaFree(d_Hx_));
	checkCudaErrors(cudaFree(d_Bx_));
	checkCudaErrors(cudaFree(d_Hy_));
	checkCudaErrors(cudaFree(d_By_));

	checkCudaErrors(cudaFree(d_Ezi));
	checkCudaErrors(cudaFree(d_Ezt));
	checkCudaErrors(cudaFree(d_Eztt));
	checkCudaErrors(cudaFree(d_Ezy1));
	checkCudaErrors(cudaFree(d_Ezy2));

	checkCudaErrors(cudaFree(d_einf_));
	checkCudaErrors(cudaFree(d_uinf_));
	checkCudaErrors(cudaFree(d_wpesq_));
	checkCudaErrors(cudaFree(d_wpmsq_));
	checkCudaErrors(cudaFree(d_ge_));
	checkCudaErrors(cudaFree(d_gm_));

	checkCudaErrors(cudaFree(d_ae0_));
	checkCudaErrors(cudaFree(d_ae_));
	checkCudaErrors(cudaFree(d_be_));
	checkCudaErrors(cudaFree(d_ce_));
	checkCudaErrors(cudaFree(d_de_));
	checkCudaErrors(cudaFree(d_ee_));
	checkCudaErrors(cudaFree(d_am0_));
	checkCudaErrors(cudaFree(d_am_));
	checkCudaErrors(cudaFree(d_bm_));
	checkCudaErrors(cudaFree(d_cm_));
	checkCudaErrors(cudaFree(d_dm_));
	checkCudaErrors(cudaFree(d_em_));

	checkCudaErrors(cudaFree(d_PsiEzX_));
	checkCudaErrors(cudaFree(d_PsiEzY_));
	checkCudaErrors(cudaFree(d_PsiHyX_));
	checkCudaErrors(cudaFree(d_PsiHxY_));
